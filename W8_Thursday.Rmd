...
```{r}
## mapview(bay_tracts) --> find census tract of interest, click it and view the GEOID, copy past that below
...
leaflet() %>% 
  ...
  addMarkers(
    # ANTIOCH coordinates
    lng = -121.78026417144844,
    lat = 37.99579272708984
  ) %>% 
  addPolygons(
    data = bay_pumas %>% 
...
# Mahalanobis matching technique:
```{r}
obs_matrix <-
  pums_bart_clean %>% 
  ungroup() %>% 
...
## DiD effect size 
The difference-in-differences analysis of this data shows that the Antioch BART station in 2018 had an estimated impact of adding approximately 1100 new BART commuters, with a standard error of 210 commuters. 

The p-value of 2.11e-6 shows us that these results are statistically significant, since the p-value < 0.05.